...
def vivodsave(filepath):
    with open(filepath, 'r', encoding='utf-8') as file:
        data = json.load(file)
        print(json.dumps(data, indent=4, sort_keys=True, ensure_ascii=False))

def delisave(filepath, inp):
...
def savejson(savefile, filepath):
    if os.path.isfile(filepath):
        with open(filepath, 'r', encoding='utf-8') as file1:
            a = [json.load(file1)]
            a.append(savefile)
            with open(filepath, 'w', encoding='utf-8') as file2:
                json.dump(a, file2, indent=2, ensure_ascii=False)
    else:
        with open(filepath, 'w', encoding='utf-8') as file2:
            json.dump(savefile, file2, indent=2, ensure_ascii=False)


def savecsv(savefile, pathway):
    savelist = list(savefile.values())
    if os.path.isfile(pathway):
        with open(pathway, 'a', newline='') as file3:
            writer = csv.writer(file3, delimiter=';')
            writer.writerow(savelist)
    else:
        with open(pathway, 'a', newline='') as file2:
            writer = csv.DictWriter(file2, fieldnames=['Data', 'name', 'akt', 'reason'], delimiter=';')
            writer.writeheader()
            with open(pathway, 'a', newline='') as file1:
...
dateGame = str(datetime.datetime.now())
pathway = 'savesAll.json'
pathwacsv = 'savesAll.csv'
magInv = ["Клинок", "Огненный посох"]
warInv = ["Экскалибур", "Деревяный щит"]
archerInv = ["Кинжал", "Лук и стрелы"]
mainInv = []
weaponDamage = []
print(blue("Игра \'Приключения в волшебном мире\'"),
      "\nЦель - пройти испытания, разгадывая загадки и побеждая монстров"
      "\n P.S. Игра на ранней стадии разработки, могут присутсвовать баги")
...
if inp == "2":
    if os.path.isfile(pathway):
        vivodsave(pathway)
        inp = input('Для удаления сохранения нажмите 1, чтобы начать игру 2')
        if inp == '1':
            inp1 = int(input('Введите номер сохраненния для удаления - '))
            delisave(pathway, inp1)
            vivodsave(pathway)
            inp2 = input('Нажмите 1 чтобы продожить')
        else:
            pass
    else:
        print('Сохранений пока нет. Игра начинается')
...
usersaves = {'DateGame' : dateGame, 'NameUser': name, 'userClass' : userclass, 'reason' :  ''}
...
if user_input == "1":
    ...
    if user_input == "1":
        ...
        usersaves['reason'] = 'Отравился водой'
        savejson(usersaves, pathway)
        exit()
    if user_input == "2":
    ...
    print("Акт 2.\"Контракт с дьяволом\""
          ...
          "\nПред вами предстает краснокаменная арка, за каторой лежит дорого из этого же кирпича"
    ...
    if user_input == "2":
        print("Вы разворачиваетесь и уходите",
              red("\n Конец игры"))
        usersaves['reason'] = 'Не начал испытание'
        savejson(usersaves, pathway)
        exit()
    if user_input == "1":
        print(
    ...
    while karahortHP > 0:
        ...
        if userHP <= 0:
            print("Вас яростно растерзал Карахорт, вы мертвы"
                  "\n GAME OVER")
            usersaves['reason'] = 'Растерзан Карахортом'
            savejson(usersaves, pathway)
            exit()
    ...
    if user_input == "1":
        ...
        usersaves['reason'] = 'Свернул не туда'
        savejson(usersaves, pathway)
        exit()
    if user_input == "2":
    ...
    print("Из земли возникают врата, вы слышате чей - то голос за ними, войдя в них у увидели..."
          "\n ПРОДОЛЖЕНИЕ СЛЕДУЕТ")
    usersaves['reason'] = 'Игра пройдена'
    savejson(usersaves, pathway)
    savecsv(usersaves, 'saves.csv')